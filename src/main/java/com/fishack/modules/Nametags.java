...
public class Nametags extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            // Only show nametag for self (player) through blocks, with max range 55
            if (type == EntityType.PLAYER && entity == mc.player) {
                if (entity.squaredDistanceTo(cameraPos) <= 55 * 55) {
                    entityList.add(entity);
                }
                continue;
            }

            // For all other entities, only show if on screen and not through blocks
            if (PlayerUtils.isWithinCamera(entity, maxCullRange.get()) && mc.world.raycast(cameraPos, entity.getPos(), BlockHitResult.Type.BLOCK) == null) {
                entityList.add(entity);
            }
        }
